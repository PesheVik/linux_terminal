...
1. `rmdir` - удалить пустую директорию, например, `rmdir test_1.test`
...
1. `wc` - считает кол-во, например, `wc -l .file.test` или `head | wc -w`
    ...
    - `w` - слов
    - `c` - символов
1. `chmod` - изменение прав доступа у `u` - пользователя, `g` - группы, `o` - всех остальных. Например, `chmod  +x`, `chmod  u-w`, `chmod u+rwx,g+rw,o-rwx`, `chmod 755`, `chmod 644`
    - `r` - чтение (4)
    - `w` - запись (2)
    - `x` - выполнение (1)
---
